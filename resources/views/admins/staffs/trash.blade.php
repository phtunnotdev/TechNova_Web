            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        ...
                        <div class="col-auto"> 
                            <form id="form_filter" method="GET" action="{{route('staff-trash.index')}}" class="row g-2">
                                <div class="col-auto">
                                    <select name="perPage" class="form-select" onchange="submitForm()">
                                        <option {{ request('perPage') == 5 ? "selected" : ""}} value="5">5 khách hàng</option>
                        ...
                        <table class="table mb-0 checkbox-all">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <form class="d-inline" action="{{route('staff-trash.update', $user->id)}}" method="POST">
                                          @csrf
                                          @method("put")
                                        ...
                                        <form class="d-inline" action="{{route('staff-trash.destroy', $user->id)}}" method="POST">
                                          @csrf
                                          @method("delete")
    ...
    function getCheckedValues() {        
        ...
        if(checkedValues.length > 0){
            if(actionSelect.value == 1){
               var type = "<?php echo route('staff.restore')?>";
               var mess = "khôi phục";
            }else{
               var type = "<?php echo route('staff.delete')?>";

                            ...
                            <form id="form_filter" method="GET" action="{{ route('user.index') }}" class="row g-2">
                                <div class="col-auto">
                                    <select name="perPage" class="form-select" onchange="submitForm()">
                                        <option {{ request('perPage') == 8 ? 'selected' : '' }} value="8">8 khách
                                            hàng</option>
                                        <option {{ request('perPage') == 10 ? 'selected' : '' }} value="10">10 khách
                                            hàng</option>
                                        <option {{ request('perPage') == 12 ? 'selected' : '' }} value="12">12 khách
                                            hàng</option>
                                        <option {{ request('perPage') == 15 ? 'selected' : '' }} value="15">15 khách
                                            hàng</option>
                                        <option {{ request('perPage') == 18 ? 'selected' : '' }} value="18">18 khách
                                            hàng</option>
                                ...
                                <div class="col-auto">
                                    <select name="status" class="form-select" onchange="submitForm()">
                                        <option {{ request('status') == 0 ? 'selected' : '' }} value="0">Trạng thái
                                        </option>
                                        <option {{ request('status') == 'active' ? 'selected' : '' }} value="active">
                                            Hoạt động</option>
                                        <option {{ request('status') == 'banned' ? 'selected' : '' }} value="banned">Bị
                                            cấm</option>
                                ...
                                <div class="col-auto">
                                    <a href="{{ route('user.create') }}"><button type="button"
                                            class="btn btn-primary"><i class="fa-solid fa-plus me-1"></i> Thêm khách
                                            hàng</button></a>
                                </div><!--end col-->
                            </form>
                        </div><!--end col-->
                            ...
                            <tbody>
                                @forelse ($users as $user)
                                    ...
                                    <tr>
                                        ...
                                        <td class="ps-0"><span
                                                class="badge bg-transparent border border-primary text-primary">{{ $user->user_code }}</span>
                                        </td>
                                        <td>
                                            <a href="{{ route('user.show', $user->id) }}">
                                                <img src="{{ $user->image ? '.' . Storage::url($user->image) : 'assets/images/users/avatar-default.png' }}"
                                                    alt="" class="thumb-md d-inline rounded-circle me-1">
                                            </a>
                                            <p class="d-inline-block align-middle mb-0">
                                                <a href="{{ route('user.show', $user->id) }}"
                                                    style="color: inherit"><span
                                                        class="font-13 fw-medium">{{ $user->name }}</span></a>
                                            </p>
                                        </td>
                                        <td><a href="#"
                                                class="d-inline-block align-middle mb-0 text-body">{{ $user->email }}</a>
                                        </td>
                                        <td class="text-center">{{ date('d/m/Y', strtotime($user->created_at)) }}</td>
                                        <td class="text-center"><span
                                                class="badge bg-primary-subtle text-primary text-capitalize">Khách
                                                Hàng</span></td>
                                        <td class="text-center"><span
                                                class="badge bg-{{ $user->status === 'active' ? 'success' : 'danger' }} text-capitalize">{{ $user->status }}</span>
                                        </td>
                                        ...
                                        <td class="text-end">
                                            ...
                                            <form class="d-inline" action="{{ route('user.destroy', $user->id) }}"
                                                ...
                                                @method('delete')
                                                <button type="submit"
                                                    onclick="return confirm('Bạn có chắc chắn chuyển vào thùng rác không ?')"
                                                    class="btn-reset"><i
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-danger">Không có khách hàng nào</td>
                                    </tr>
                                @endforelse
                            </tbody>
                            <tfoot>
                                <tr>
                                    ...
                                    <th class="ps-0" colspan="2">
                                        <select class="form-select w-auto" id="action-select"
                                    ...
                                    <th colspan="4"></th>
                                    <th class="text-end"><a href="{{ route('user-trash.index') }}"><button
                                                class="btn btn-danger py-1 px-2"><i
